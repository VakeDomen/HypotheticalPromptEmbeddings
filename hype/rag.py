...
# Function to generate diverse questions for a chunk
def generate_questions(chunk, num_questions=5):
    prompt = f"Generate diverse questions that can be answered using the following chunk of text:\n\n{chunk}\n\nQuestions:"
...
for doc_id, item in tqdm(enumerate(chunked_data), total=len(chunked_data)):
    for chunk_id, chunk in enumerate(item['chunks']):
        # Generate multiple diverse questions for each chunk
        questions = generate_questions(chunk, num_questions=5)
        for question in questions:
            questions_for_embedding.append(question)
            # Store the original chunk and metadata for retrieval later
            texts.append(chunk)
            metadata.append({
